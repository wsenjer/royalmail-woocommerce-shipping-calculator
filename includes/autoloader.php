...
spl_autoload_register(function( $filename ) {
    ...
    if (isset($file_path[0]) && $file_path[0] !== 'wprubyroyalmail') {
    ...
    if ( isset( $file_path[ count( $file_path ) - 1 ] ) ) {
        $original_file_name = $original_file_path[ count( $original_file_path ) - 1 ];
        ...
        if ( false !== $index ) {
            ...
            // if the class is in Deps folder, we do not follow PSR-4, not WP naming convention.
        } elseif(isset($file_path[1]) && $file_path[1] === 'build') {
